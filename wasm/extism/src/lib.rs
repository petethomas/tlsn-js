...
struct WebRequestOptions {
    ...
    body: String,
    maxTranscriptSize: u32,
    secretHeaders: Vec<String>,
    secretResps: Vec<String>,
}

#[derive(serde::Deserialize)]
struct ProverInput {
    target_url_str: String,
    val: WebRequestOptions,
    secret_headers: Vec<String>,
    secret_body: Vec<String>,
}

#[plugin_fn]
pub async fn prover(Json(proverInput): Json<ProverInput>) -> FnResult<String> {
    log!("target_url: {}", proverInput.target_url_str);
    let target_url = Url::parse(&proverInput.target_url_str[..])
        .map_err(Error::msg)?;
    ...
    let options: RequestOptions = serde_wasm_bindgen::from_value(proverInput.val)
    ...
    */
    ...
    let mut req_with_header = Request::builder()
        .uri(proverInput.target_url_str)
    ...
    let secret_headers_vecs = string_list_to_bytes_vec(proverInput.secret_headers)?;
    ...
    let secret_body_vecs = string_list_to_bytes_vec(proverInput.secret_body)?;
